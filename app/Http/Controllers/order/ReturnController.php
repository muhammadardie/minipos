...
use App\Models\order\Purchase_order_return;
...
class ReturnController extends Controller
{
    ...
    */
        ...
        if($request->ajax()){
            ...
            if($request->ajax()){
                $purchase_order = Purchase_order::select([
                                    ...
                                    ->orWhere('purchasing_orders.approved', FALSE);
                                        ...
                                        if(count($permission) > 1){
                                            ...
                                            if(array_key_exists('edit', $permission) && $purchase_order->approved === null){
                                                if (method_exists($controller, 'show') && is_callable(array($controller, 'edit'))){
                                                    $btn_action .= '<a href="#" data-url="'. route('order.return.ajax_return_order') .'" data-id="'. $purchase_order->id .'" class="dropdown-item set-return"><i class="la la-edit"></i>Set return order</a>';
                                                }
                                            }
    ...
    public function ajax_return_order(Request $request)
    {
        if($request->ajax()){
            $id = $request->id;
            ...
            try {
                ...
                foreach ($purchase->purchase_order_detail as $dtl) {
                    // insert to receive
                    $purchase_return                             = new Purchase_order_return;
                    $purchase_return->po_number                  = $purchase->po_number;
                    $purchase_return->employee_id                = \Auth::user()->employee_id;
                    $purchase_return->purchasing_order_detail_id = $dtl->id;
                    $purchase_return->total                      = $dtl->total;
